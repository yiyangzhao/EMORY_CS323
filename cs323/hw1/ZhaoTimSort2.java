...
public class ZhaoTimSort2<T extends Comparable<T>> extends MergeSort<T>
{
	...
	public void sort(T[] array, int beginIndex, int endIndex)
	{
		...
		{
			SheSort(array, beginIndex, endIndex);
		}
	}
	
	private void SheSort(T[] array, int beginIndex, int endIndex) {
		int h = getMaxH(endIndex - beginIndex);
		
		while (h >= 1)
		{
			//Insetion sort with the gap of h
			SheSort(array, beginIndex, endIndex, h);
			h = getNextH(h);
		}
		
	}
	private void SheSort(T[] array, int beginIndex, int endIndex, int h) {
